        ...
        while True:
            ...
            if (isinstance(data_object, RequestVoteResponse)):
                ...
                if (isinstance(self.currentState, Candidate)):
                    ...
                    elif self.currentTerm > data_object.currentTerm:
                        reqVoteMsg = RequestVote(
                            self.currentTerm, self.id, serverConfig.SERVER_PORTS[data_object.sender], self.id,
                            self.lastLogIndex, self.lastLogTerm
                        )
                        print(f"reqVoteMsg, RequestVote is {isinstance(reqVoteMsg, RequestVote)}")
                        if isinstance(reqVoteMsg, RequestVote):
                            self.currentState.sendReqVoteMessage(reqVoteMsg)
                elif isinstance(self.currentState, Follower):
                    self.currentState.respondToRequestVote(self, data_object)
            elif (isinstance(data_object, AppendEntry)):
                # print(" Get into AppendEntry")
                self.currentState = Follower()
                ...
                # print("Get into AcceptAppendEntry")
                # print(f"nowterm at AcceptAppendEntry:{data_object.currentTerm}")
                if not data_object.acceptEntry:
                    if data_object.currentTerm > self.currentTerm:
